...
document.addEventListener("DOMContentLoaded", () => {
    try {
        ...
        // Render the page immediately. Do not wait for auth/session.
        loadPage("dashboard", false);

        // Session status is only a background UI update.
        updateSessionUI();
    } catch (error) {
        console.error("SentinelAI startup error:", error);
        ...
        if (pageContent) {
            pageContent.innerHTML = `
                <div class="empty-state">
                    <h3>Unable to start SentinelAI</h3>
